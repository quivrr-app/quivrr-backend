...
INPUT_FILE = Path("scrapers/manufacturers/availability/output/album/album_au_manufacturer_inventory.json")
...
def main():
    print("")
    print("Importing Album AU manufacturer availability")
    ...
    with engine.begin() as conn:
        ...
        for row in rows:
            ...
            length = clean(row.get("lengthFeetInches") or row.get("length"))
            volume = row.get("volumeLitres")
            construction = clean(row.get("construction")) or "Standard"
            ...
            """), {
                ...
                "brand_name": clean(row.get("brandName")) or "Album",
                "model_name": model_name,
...
if __name__ == "__main__":
    try:
        ...
        print(f"Album manufacturer availability import failed: {exc}")
        sys.exit(1)
