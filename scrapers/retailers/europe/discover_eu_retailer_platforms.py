...
INPUT_FILE = Path("scrapers/retailers/europe/eu_retailer_targets.json")
OUTPUT_FILE = Path("scrapers/retailers/europe/output/eu_retailer_discovery.json")
TIMEOUT_SECONDS = 12
MAX_BYTES = 350_000
...
USER_AGENT = (
    ...
    "Chrome/137.0 Safari/537.36 QuivrrEUDiscovery/1.0"
)
...
def detect_platform_from_text(text: str) -> dict:
    checks = [
        (
            "shopify",
            "needs_review",
            ["cdn.shopify.com", "shopify.theme", "myshopify.com", "/cart/add"],
        ),
        (
            "woocommerce",
            "needs_review",
            ["woocommerce", "wp-content/plugins/woocommerce", "wc-blocks"],
        ),
        (
            "prestashop",
            "needs_adapter",
            ["prestashop", "powered by prestashop", "/modules/ps_"],
        ),
        (
            "magento",
            "needs_adapter",
            ["magento", "mage/", "Magento_Ui".lower()],
        ),
        (
            "bigcommerce",
            "needs_adapter",
        ...
        (
            "wix",
            "needs_adapter",
            ["wix.com", "static.wixstatic.com", "_wixcssrules"],
        ),
        (
            "squarespace",
            "needs_adapter",
        ...
        (
            "lightspeed",
            "needs_adapter",
            ["lightspeed", "ecwid-lightspeed"],
        ),
        (
            "shopware",
            "needs_adapter",
            ["shopware", "sw-cache-hash", "storefront/js"],
        ),
    ]
    ...
    score = sum(1 for marker in catalogue_markers if marker in text)

    if score >= 2:
        return {
            "platform": "generic_catalogue",
            "status": "needs_review",
            "evidence": f"Detected {score} generic catalogue markers",
        }
...
def main() -> None:
    ...
    report = {
        ...
        "purpose": "EU retailer platform discovery only. No SQL import, no production table writes.",
        "regionCode": "EU",
        "priceCurrency": "EUR",
        "ukExcluded": True,
    ...
    print("EU retailer discovery complete")
