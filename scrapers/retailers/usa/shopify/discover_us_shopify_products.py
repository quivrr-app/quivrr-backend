...
from scrapers.retailers.usa.normalise_us_retailer_inventory import normalise_row


INPUT_FILE = Path("scrapers/retailers/usa/shopify/us_shopify_targets.json")
OUTPUT_FILE = Path(
    "scrapers/retailers/usa/shopify/output/us_shopify_product_discovery.json"
)
...
HEADERS = {
    "User-Agent": (
        ...
        "Chrome/137.0 Safari/537.36 QuivrrUSShopifyDiscovery/1.0"
    ),
...
def discover_target(target: dict, max_pages: int) -> dict:
    if target.get("regionCode") != REGION_CODE:
        raise RuntimeError(
            f"US Shopify discovery requires RegionCode 'US', got {target.get('regionCode')!r}."
        )
...
def build_dry_run_report(targets: list[dict]) -> dict:
    return {
        ...
        "mode": "dry_run",
        "purpose": "US Shopify product discovery scaffold. No network fetches, SQL writes, imports, or production file writes.",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Discover likely US Shopify surfboard products without importing to SQL."
    )
    parser.add_argument(
        "--run-enabled",
        action="store_true",
    ...
    if not args.run_enabled:
        ...
        report = {
            "generatedAtUtc": datetime.now(timezone.utc).isoformat(),
            "mode": "run_enabled",
            "purpose": "US Shopify product discovery only. No SQL import and no production table writes.",
            "regionCode": "US",
    ...
    print("US Shopify product discovery")
