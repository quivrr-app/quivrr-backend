...
DISCOVERY_FILES = [
    ...
    Path("scrapers/retailers/usa/custom/output/us_custom_product_discovery.json"),
...
def normalise_row(row: dict) -> dict:
    ...
    if source_region != REGION_CODE:
        raise RuntimeError(
            "US normalisation safety failed for "
            f"{row.get('retailerSlug', '<missing>')}: RegionCode must be 'US', "
...
def run_self_tests() -> None:
    ...
    print("Generic US normaliser self-tests passed")


def main() -> None:
    parser = argparse.ArgumentParser(description="Normalise US retailer discovery outputs without SQL writes.")
    ...
    report = {
        "purpose": "Generic US retailer inventory normalisation only. No SQL import or RetailerInventory writes.",
        "inputFiles": sources,
        "regionCode": REGION_CODE,
    ...
    print("Generic US normalisation complete")
