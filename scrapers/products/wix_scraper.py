...
OUTPUT_DIR = Path("scrapers/products/output/wix")
...
def main():
    ...
    wix_retailers = [
        ...
        if retailer.get("platform") == "wix"
    ]

    print(f"Wix retailers: {len(wix_retailers)}")
    print("")
    ...
    for index, retailer in enumerate(wix_retailers, start=1):
        print(f"[{index}/{len(wix_retailers)}] {retailer['primary_name']}")

        products = scrape_wix(retailer)
