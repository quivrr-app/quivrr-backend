...
OUTPUT_DIR = Path("scrapers/products/output/ecwid")
...
def main():
    ...
    ecwid_retailers = [
        ...
        if retailer.get("platform") == "ecwid"
    ]

    print(f"Ecwid retailers: {len(ecwid_retailers)}")
    print("")
    ...
    for index, retailer in enumerate(ecwid_retailers, start=1):
        print(f"[{index}/{len(ecwid_retailers)}] {retailer['primary_name']}")

        products = scrape_ecwid(retailer)
