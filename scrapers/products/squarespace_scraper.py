...
OUTPUT_DIR = Path("scrapers/products/output/squarespace")
...
def is_available(offer, html):
    ...
    return not any(term in html_lower for term in unavailable_terms)
    ...
    if products:
        ...
        for product in products:
            ...
            output.append({
                ...
                "price": price,
                "compare_at_price": "",
                "available": is_available(offer, html),
...
def scrape_squarespace(retailer):
...
def main():
    ...
    squarespace_retailers = [
        retailer
        for retailer in retailers
        if retailer.get("platform") == "squarespace"
    ]

    print(f"Squarespace retailers: {len(squarespace_retailers)}")
    print("")
    ...
    for index, retailer in enumerate(squarespace_retailers, start=1):
        print(f"[{index}/{len(squarespace_retailers)}] {retailer['primary_name']}")

        products = scrape_squarespace(retailer)
